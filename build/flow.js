	...
	class Canvas extends Serializer {
	  constructor() {
	    ...
	    const onZoom = e => {
	      if (e.touches) {
	        if (e.touches.length === 2) {
	          ...
	          if (zoom < .52) zoom = .5;else if (zoom > .98) zoom = 1; //contentDOM.style.left = numberToPX( this.centerX / zoom );
	    ...
	    this._onMoveEvent = e => {
	      ...
	      this.clientY = event.clientY; //this.relativeClientX = ( ( ( dom.scrollLeft - this.centerX ) + event.clientX ) - rect.left ) / zoom;
	  ...
	  updateMap() {
	    ...
	    const bounds = this.getBounds();
	    if (canvas.width > canvas.height) canvas.height / canvas.width;else canvas.width / canvas.height;
	    mapCanvas.width = 300;

...
(function (global, factory) {
	  ...
	  constructor(draggable = true) {
	    ...
	    const onDrag = e => {
	      ...
	      if (this.draggable === true) {
	        draggableDOM(this.node.dom, data => {
	...
	class Canvas extends Serializer {
	  ...
	  isNodeVisible(node) {
	    ...
	    const bounds = node.getBounds(); // Calculate visible area in screen coordinates

	    const viewLeft = 0;
	    const viewTop = 0;
	    const viewRight = _width / zoom;
	    const viewBottom = _height / zoom; // Node boundaries in screen coordinates (getBounds already includes scrollLeft/scrollTop)

	    const nodeLeft = bounds.x;
	    const nodeRight = bounds.x + bounds.width;
	    const nodeTop = bounds.y;
	    const nodeBottom = bounds.y + bounds.height; // Check if node intersects with visible area (any part visible)

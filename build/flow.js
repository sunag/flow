...
(function (global, factory) {
	...
	class Serializer extends EventTarget {
	  ...
	  deserializeLib() {// Abstract function.
	  }
	...
	const rgbaToArray = rgba => {
	  const values = rgba.substring(rgba.indexOf('(') + 1, rgba.indexOf(')')).split(',').map(num => parseInt(num.trim()));
	  return values;
	};
	...
	class Element extends Serializer {
	  ...
	  disconnect(element) {
	    ...
	    const {
	      disconnectDOM
	    } = connections.get(element); // remove the current input

	    disconnectDOM.dispatchEvent(new Event('disconnect')); // re-update the connections
	  ...
	  constructor(draggable = true) {
	    ...
	    const onDrag = e => {
	      ...
	      if (this.draggable === true) {
	        draggableDOM(this.node.dom, null, {
	...
	class Canvas extends Serializer {
	  ...
	  constructor() {
	    ...
	    draggableDOM(mapCanvas, data => {
	      ...
	      if (data.scrollLeft === undefined) {
	        ...
	        const overMapScreen = clientMapX > screen.x && clientMapY > screen.y && clientMapX < screen.x + screen.width && clientMapY < screen.y + screen.height;
	  ...
	  isNodeVisible(node) {
	    ...
	    const height = node.getHeight(); // Calculate visible area in world coordinates

	    const viewLeft = -scrollLeft;
	    const viewTop = -scrollTop;
	    const viewRight = viewLeft + _width / zoom;
	    const viewBottom = viewTop + _height / zoom; // Node boundaries in world coordinates

	    const nodeLeft = position.x;
	    const nodeRight = position.x + width;
	    const nodeTop = position.y;
	    const nodeBottom = position.y + height; // Check if node intersects with visible area (any part visible)

	    const isVisible = !(nodeRight < viewLeft || nodeLeft > viewRight || nodeBottom < viewTop || nodeTop > viewBottom);
	    return isVisible;
	  }

	  updateNodesVisibility() {
	    const visibleNodes = new Set(); // First pass: identify directly visible nodes
	    ...
	    for (const link of links) {
	      if (link.inputElement && link.outputElement) {
	        ...
	        const outputNode = link.outputElement.node; // If one node is visible, make sure the connected node is also visible
	...
	class LabelElement extends Element {
	  ...
	  setIcon(value) {
	    ...
	    this.iconDOM.className = value;
	    if (value) this.labelDOM.prepend(this.iconDOM);else this.iconDOM.remove();
	    return this;
	  }
	...
	class ButtonInput extends Input {
	  ...
	  constructor(innterText = '') {
	    ...
	    dom.onclick = dom.ontouchstart = iconDOM.onclick = iconDOM.ontouchstart = e => {
	      e.preventDefault();
	      e.stopPropagation();
	...
	class StringInput extends Input {
	  ...
	  setIcon(value) {
	    ...
	    this.iconDOM.className = value;
	    if (value) this.dom.prepend(this.iconDOM);else this.iconDOM.remove();
	    return this;
	  }
	...
	class TreeViewNode {
	  ...
	  setSelected(value) {
	    ...
	    if (value) this.dom.classList.add('selected');else this.dom.classList.remove('selected');
	    this.selected = value;
	    return this;
	  }
	...
	class Loader extends EventTarget {
	  ...
	  async load(url, lib = {}) {
	    return await fetch(url).then(response => response.json()).then(result => {
	...
	exports.TreeViewInput = TreeViewInput;
